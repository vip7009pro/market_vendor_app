...
class _SalesHistoryScreenState extends State<SalesHistoryScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final filtered = _range == null
        ? sales
        : sales.where((s) => s.createdAt.isAfter(_range!.start) && s.createdAt.isBefore(_range!.end)).toList();
    ...
    return Scaffold(
      ...
      body: ListView.separated(
        ...
        itemBuilder: (context, i) {
          final s = filtered[i];
          final subtitle = s.items.map((e) => '${e.name} x ${e.quantity}').join(', ');
          return ListTile(
            title: Text(currency.format(s.total)),
            subtitle: Text('${fmtDate.format(s.createdAt)} • $subtitle'),
            trailing: Row(
